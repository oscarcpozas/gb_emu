...
use crate::{filters, Error, Generate, Result};
...
fn get_instructions(file: &File) -> Vec<Instruction> {
    ...
    let unprefixed_inst: Vec<Instruction> = instructions
        .unprefixed
        .into_iter()
        .map(|mut inst| {
        ...
        .collect();
    let cb_inst: Vec<Instruction> = instructions
        .cbprefixed
        .into_iter()
        .map(|mut inst| {
            inst.code.insert_str(2, "cb");
            inst
        })
        .collect();
